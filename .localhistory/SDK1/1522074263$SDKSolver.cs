...
namespace SDK1
{
    public partial class SDKSolver : Form
    {
        public SDKSolver()
        {
            InitializeComponent();
        ...
        private bool CheckCell(int[,] sudokuArray, int row, int column)
        {
            ...
            if(valuesFoundCount == 1)
            {
                ...
                MethodInvoker methodInvokerChangeValues = delegate ()
                {
                    ...
                    dgvSudok.Rows[row].Cells[column].Style.BackColor = Color.Green;
                };
        ...
        private bool SolveEvilLoop()
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                for (int f = 0; f < 9; f++)
                {
                    if (CheckCell(sudokuArray, i, f)) return true;
            ...
            for (int values = 0; values < 9; values++)
            {
                ...
                if (valuesFoundCount == 1)
                {
                    ...
                    MethodInvoker methodInvokerChangeValues = delegate ()
                    {
                        ...
                        dgvSudok.Rows[row].Cells[columnPosition].Style.BackColor = Color.Yellow;
                    };
                    if (InvokeRequired) BeginInvoke(methodInvokerChangeValues);
                    else BeginInvoke(methodInvokerChangeValues);
                    return true;
                ...
                for (int values = 0; values < 9; values++)
                {
                    ...
                    if (valuesFoundCount == 1)
                    {
                        ...
                        MethodInvoker methodInvokerChangeValues = delegate ()
                        {
                            ...
                            dgvSudok.Rows[rowPosition].Cells[columnPosition].Style.BackColor = Color.Blue;
                        };
                        if (InvokeRequired) BeginInvoke(methodInvokerChangeValues);
                        else BeginInvoke(methodInvokerChangeValues);
                        return true;
        ...
        private bool CheckCellColumn(int[,,] fullArray)
        {
            for (int columnCount = 0; columnCount < 9; columnCount++)
            {
                ...
                for (int i = 0; i < 9; i++)
                {
                    for (int f = 0; f < 9; f++)
                    {
                        columnarray[i, f] = fullArray[i, columnCount, f];
                    }
                }
                for (int values = 0; values < 9; values++)
                {
                    ...
                    {
                        string valuefound = (values + 1).ToString();
                        int columnPosition = columnCount;
                        int rowPosition = Array.IndexOf(possibleValues, 0);
                        MethodInvoker methodInvokerChangeValues = delegate ()
                        {
                            lblProgress.Text = $"Progress: Value found for Column {columnPosition} and Row {rowPosition}";
                            dgvSudok.Rows[rowPosition].Cells[columnPosition].Value = valuefound;
                            dgvSudok.Rows[rowPosition].Cells[columnPosition].Style.BackColor = Color.Red;
                        };
                        if (InvokeRequired) BeginInvoke(methodInvokerChangeValues);
                        else BeginInvoke(methodInvokerChangeValues);
                        return true;
