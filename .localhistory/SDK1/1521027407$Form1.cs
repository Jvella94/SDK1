...
namespace SDK1
{
    public partial class Form1 : Form
    {
        ...
        private bool CheckCellBox(int[,,] fullArray, int rowsDone)
        {
            ...
            for (int g = 0; g < 3; g++)
            {
                ...
                for (int i = 0; i < 9; i++)
                {
                    for (int f = 0; f < 9; f++)
                    {
                        int rowvalue = boxRowValue + (i % 3);
                        int columnvalue = (g * 3) + (i / 3);
                ...
                for (int values = 0; values < 9; values++)
                {
                    ...
                    {
                        string valuefound = (values + 1).ToString();
                        int columnPosition = (g * 3) + (values / 3);
                        int rowPosition = boxRowValue + (values % 3);
                        MethodInvoker methodInvokerChangeValues = delegate ()
                        {
        ...
        private void PrintArray(int[,,] fullArray)
        {
            for (int i = 0; i < 10; i++)
            {
                File.AppendAllText(Environment.CurrentDirectory + "\\logfile.txt", (i).ToString() + ",");
            }
            File.AppendAllText(Environment.CurrentDirectory + "\\logfile.txt", Environment.NewLine);
            for (int i = 0; i < 9; i++)
            {
                File.AppendAllText(Environment.CurrentDirectory + "\\logfile.txt", (i+1).ToString() + ",");
                for (int f = 0; f < 9; f++)
                {
                    for (int g = 0; g < 9; g++)
                    {
